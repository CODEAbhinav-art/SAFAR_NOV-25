...
/**
 * WalletTopUpDialog.java
 * ----------------------
 * Allows logged-in user to add money to their wallet.
 * Updates the database (users.wallet_balance) and refreshes dashboard label.
 */
public class WalletTopUpDialog extends JDialog implements ActionListener {
    ...
    public WalletTopUpDialog(JFrame parent, String username, JLabel walletLabel) {
        ...
        if (e.getSource() == cancelButton) {
            ...
            try {
                ...
                if (updateWalletBalance(amount)) {
                    ...
                    // Refresh wallet label live
                    double newBalance = getCurrentBalance();
                    walletLabel.setText("Current Balance: ₹" + String.format("%.2f", newBalance));
                    dispose();
                } else {
                    JOptionPane.showMessageDialog(this, "Failed to update wallet. Try again.", "Error", JOptionPane.ERROR_MESSAGE);
                }
    ...
    private boolean updateWalletBalance(double amount) {
        try (Connection conn = DBConnection.getConnection()) {
            String update = "UPDATE users SET wallet_balance = wallet_balance + ? WHERE username = ?";
    ...
    /**
     * Fetches current balance for refresh
     */
